      ...
      <CardContent>
        <ul className="space-y-2">
          {recipesToMake.map((recipe, index) => (
            <li key={`${(recipe.title || recipe.name)}-${index}`} className="flex justify-between items-center p-2 rounded hover:bg-muted/50">
              ...
              <Button
                ...
                onClick={() => {
                  // Pass a single object matching the expected ToMakeRecipeItem structure
                  onRemoveRecipe({
                    title: recipe.title,
